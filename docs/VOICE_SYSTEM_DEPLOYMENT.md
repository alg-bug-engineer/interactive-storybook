# 音色系统部署与测试指南

## 一、已完成的功能

### Phase 0: PRD 文档 ✅
- ✅ 产品需求文档（PRD_VOICE_SYSTEM.md）
- ✅ 音色列表定义（8个音色）
- ✅ 技术架构设计
- ✅ 成功指标定义

### Phase 1: 核心功能 ✅
- ✅ 后端音色服务（TTS 基于 edge-tts）
- ✅ 音色 API（列表、试听、偏好保存）
- ✅ 用户偏好持久化（前端 LocalStorage + 后端用户配置）
- ✅ 前端音色状态管理（Zustand）
- ✅ 音色选择器组件
- ✅ 音频播放器组件（倍速、进度条）

### Phase 2: 交互与 UI 打磨 ✅
- ✅ 音色卡片展示（推荐 vs 更多音色）
- ✅ 试听功能（一键播放示例音频）
- ✅ 选中状态高亮
- ✅ 播放器倍速控制（0.75x - 2x）
- ✅ 进度条拖拽
- ✅ 时长显示

## 二、项目结构

### 后端新增文件
```
backend/
├── app/
│   ├── constants/
│   │   ├── __init__.py
│   │   └── voices.py              # 音色常量定义
│   ├── routers/
│   │   ├── audio.py                # 音频文件服务
│   │   └── voices.py               # 音色 API
│   └── services/
│       └── tts_service.py          # TTS 语音合成服务
├── data/
│   └── audio/
│       ├── preview/                # 预览音频存储
│       └── tts/                    # TTS 音频缓存
└── requirements.txt                # 新增 edge-tts>=6.1.0
```

### 前端新增文件
```
frontend/
└── src/
    ├── components/
    │   ├── AudioPlayer.tsx         # 音频播放器组件
    │   └── VoiceSelector.tsx       # 音色选择器组件
    ├── stores/
    │   └── voiceStore.ts           # 音色状态管理
    └── services/
        └── api.ts                  # 新增音色相关 API
```

## 三、部署步骤

### 1. 后端部署

#### 安装依赖
```bash
cd backend
pip install -r requirements.txt
```

#### 验证 edge-tts 安装
```bash
edge-tts --list-voices | grep "zh-CN"
```

应该看到 8 个中文音色：
- zh-CN-XiaoxiaoNeural
- zh-CN-XiaoyiNeural
- zh-CN-YunjianNeural
- zh-CN-YunxiNeural
- zh-CN-YunxiaNeural
- zh-CN-YunyangNeural
- zh-CN-liaoning-XiaobeiNeural
- zh-CN-shaanxi-XiaoniNeural

#### 启动后端
```bash
cd backend
uvicorn app.main:app --reload --host 0.0.0.0 --port 8100
```

### 2. 前端部署

#### 安装依赖（如需要）
```bash
cd frontend
npm install
```

#### 启动前端
```bash
npm run dev
```

## 四、功能测试清单

### 4.1 音色选择器测试
- [ ] 打开主页，点击右上角"🎙️ 音色"按钮
- [ ] 查看是否显示 3 个推荐音色（晓晓、晓伊、云健）
- [ ] 查看"更多音色"区域是否显示其他 5 个音色
- [ ] 点击任意音色的"▶️ 试听"按钮
- [ ] 确认能听到预览音频（约 10 秒）
- [ ] 点击其他音色卡片，确认选中状态切换
- [ ] 关闭音色选择器，重新打开，确认选中状态保持

### 4.2 音色持久化测试
- [ ] 选择一个非默认音色（如"晓伊"）
- [ ] 刷新页面，确认右上角显示"晓伊"
- [ ] 打开音色选择器，确认"晓伊"仍被选中
- [ ] 登录账号后，切换音色
- [ ] 退出登录，重新登录，确认音色设置保持

### 4.3 播放器测试（暂未集成到故事播放）
- [ ] 音频播放/暂停
- [ ] 进度条拖拽
- [ ] 倍速切换（0.75x / 1x / 1.25x / 1.5x / 2x）
- [ ] 时长显示准确

### 4.4 API 测试

#### 获取音色列表
```bash
curl http://localhost:8100/api/voices/list
```

预期返回：
```json
{
  "voices": [...],
  "default_voice_id": "zh-CN-XiaoxiaoNeural",
  "tts_available": true
}
```

#### 试听音色
```bash
curl http://localhost:8100/api/voices/preview/zh-CN-XiaoxiaoNeural
```

预期返回：
```json
{
  "voice_id": "zh-CN-XiaoxiaoNeural",
  "audio_url": "/api/audio/data/audio/preview/zh-CN-XiaoxiaoNeural.mp3",
  "voice_info": {...}
}
```

#### 保存用户偏好（需登录）
```bash
curl -X POST http://localhost:8100/api/voices/preferences \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -d '{"preferred_voice":"zh-CN-XiaoyiNeural","playback_speed":1.25}'
```

## 五、下一步集成工作（后续开发）

### 5.1 故事 TTS 集成
需要修改的文件：
- `backend/app/services/story_engine.py`
- `frontend/src/components/StoryScreen.tsx`

功能点：
- 故事段落生成时调用 TTS 生成音频
- 前端播放 TTS 音频（替换浏览器 Web Speech API）
- 音色参数透传到 TTS 请求

### 5.2 画廊故事音色覆盖
- 从画廊加载故事时，使用当前全局音色重新生成 TTS
- 或者：在播放时动态调用 TTS API

### 5.3 倍速记忆
- 已实现前端倍速持久化
- 需要在播放器中应用倍速设置

## 六、常见问题

### Q1: edge-tts 安装失败
**解决方案：**
```bash
# macOS/Linux
pip install --upgrade edge-tts

# Windows
pip install --upgrade edge-tts --user
```

### Q2: 预览音频生成慢
**原因：** 首次访问时需要生成预览音频

**解决方案：**
- 后端启动时会自动预生成 3 个推荐音色的预览
- 可手动触发预生成：
```bash
curl -X POST http://localhost:8100/api/voices/preview/zh-CN-XiaoxiaoNeural
```

### Q3: 音频文件 404
**检查：**
1. `backend/data/audio/` 目录是否存在
2. 音频文件权限是否正确
3. 后端日志中是否有 TTS 生成错误

### Q4: 跨域问题
**检查：**
- `backend/app/config.py` 中的 CORS 配置
- 前端 API_URL 是否正确

## 七、性能优化建议

### 7.1 音频缓存
- ✅ 已实现：预览音频缓存
- ✅ 已实现：TTS 音频按 story_id + segment_index + voice_id 缓存
- 建议：定期清理过期音频文件

### 7.2 预加载
- ✅ 已实现：后台预生成推荐音色预览
- 建议：预加载常用音色的示例音频

### 7.3 CDN 加速
- 建议：将音频文件上传到 CDN（如阿里云 OSS）
- 减少服务器带宽压力

## 八、数据埋点（已定义，待实施）

```typescript
// 音色相关
trackEvent('voice_preview_played', { voice_id, duration })
trackEvent('voice_selected', { voice_id, source })
trackEvent('voice_switch', { from_voice, to_voice })

// 播放相关
trackEvent('playback_started', { story_id, segment_index, voice_id })
trackEvent('playback_speed_changed', { speed, story_id })
trackEvent('seek_used', { from_time, to_time, story_id })
trackEvent('playback_completed', { story_id, duration })

// TTS 相关
trackEvent('tts_requested', { story_id, segment_index, voice_id })
trackEvent('tts_success', { story_id, segment_index, voice_id, latency })
trackEvent('tts_failed', { story_id, segment_index, voice_id, error })
```

## 九、安全注意事项

### 9.1 文件路径安全
- ✅ 已实现：音频文件路径验证（防止路径遍历攻击）
- ✅ 已实现：文件名安全检查

### 9.2 音色验证
- ✅ 已实现：音色 ID 白名单验证
- ✅ 已实现：无效音色自动回退到默认值

### 9.3 用户偏好
- ✅ 已实现：倍速范围限制（0.5 - 2.0）
- ✅ 已实现：未登录时使用本地缓存

---

**版本**: v1.0  
**完成日期**: 2026-02-10  
**开发状态**: Phase 0-2 已完成，Phase 3（增长功能）待开发
